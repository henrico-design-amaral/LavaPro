...
function orderCard(item) {
  return `<li class="row-card"><div><strong>${item[1]} <span>${item[2]}</span></strong><p>${item[3]} · ${item[4]}</p></div><div class="right"><b>${money.format(item[6])}</b>${pill(item[5])}</div></li>`;
}
...
function renderServicos() {
  return `<section class="view"><div class="three-col">${services.map((item) => panel(item[0], item[3], `<div class="signal-grid compact"><span><b>${money.format(item[1])}</b> preço</span><span><b>${item[2]}</b> duração</span></div>`)).join("")}</div></section>`;
}
...
function render() {
  ...
  document.title = `LavaPro — ${views[view]}`;
  document.querySelector("#view-title").textContent = views[view];
...
function bindDemoActions() {
  document.querySelectorAll("[data-demo-action]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      const message = action === "create" ? "Prévia estática: nenhuma OS foi gravada. Rode o MVP local para persistência real." : "Simulação estática: sem backend, sem banco e sem sincronização real.";
      showToast(message);
    });
  });
}
...
window.LavaProStaticDemo = {
  negocio: "LavaPro Centro",
  contadores: { negocios: 1, clientes: 8, veiculos: 10, servicos: 6, produtos: 8, ordens: 12, movimentos: 34 },
};
